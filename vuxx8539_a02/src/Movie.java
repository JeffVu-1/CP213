...
public class Movie implements Comparable<Movie> {
	...
	StringBuilder formattedGenres = new StringBuilder();
        ...
        for (int i = 0; i < GENRES.length; ++i) {
            formattedGenres.append(String.format("%2d: %s\n", i, GENRES[i]));
        }
	...
	int compareValue = this.title.compareToIgnoreCase(target.title);
	if (compareValue != 0) {
	    return compareValue;
	}
	...
	FormatedString.append(String.format("Title:    %s\n", title));
	FormatedString.append(String.format("Year:     %d\n", year));
	FormatedString.append(String.format("Director: %s\n", director));
	FormatedString.append(String.format("Rating:   %.1f\n", rating));
	FormatedString.append(String.format("Genre:    %s\n", genreToName())); 
	...
	String formattedData = String.format("%s|%d|%s|%.1f|%s", title, year, director, rating, genreToName());
