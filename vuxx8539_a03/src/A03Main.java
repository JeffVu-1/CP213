...
public class A03Main {
	...
	//append all of the values from the random int
	...
	//looks like it's grabbing random values to prepare to push onto the stack 
	Integer[] testValues = getValues();
	System.out.println(LINE);
	System.out.println("SingleStack<Integer> source = new SingleStack<>();");
	//creating a new stack array of object integers
	final SingleStack<Integer> source = new SingleStack<>();
	//checking if it's empty
	System.out.println("  isEmpty {true}: " + source.isEmpty());
	System.out.println(LINE);

	//print out all of the values that are going to be pushed onto the stack
	System.out.println("Push objects: " + Arrays.toString(testValues));
	//enchanced for loop to push all the objects into the stack
	for (Integer object : testValues) {
		//print out the object that is being pushed onto the stack
	...
	System.out.println("  isEmpty {false}: " + source.isEmpty());
	//print what the peak value should be by indexing into the Testvalues array, then print out the actual peak value
	System.out.println(String.format("  peek {%d}: %d", testValues[testValues.length - 1], source.peek()));
	//print out the contents of the stack
	System.out.println("  Contents: " + objectsToString(source));
	System.out.println(LINE);

	//testing the splitAlternate method
	System.out.println("source.splitAlternate(left, right)");
	//create two new stacks to merge into the orginal stack
	final SingleStack<Integer> left = new SingleStack<>();
	final SingleStack<Integer> right = new SingleStack<>();
	//now call the split method to merge them onto the original stack
	source.splitAlternate(left, right);
	//check if the orginal source stack is empty, which it should not be in this case
	System.out.println("source");
	System.out.println("  isEmpty {true}: " + source.isEmpty());
	//then print all the items in the soure stack
	System.out.println("  Contents: " + objectsToString(source));
	//check if all the values of the left stack was moved to the source stack, by seeing if it's empty.
	System.out.println("left"); 
	System.out.println("  isEmpty {false}: " + left.isEmpty());
